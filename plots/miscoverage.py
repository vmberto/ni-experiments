...
def plot_results_all_dynamic(df_in, df_out, x_label='Mean Centered F-Score', figsize=(50, 25)):
    ...
    for row_idx, model in enumerate(models):
        ...
        for col_idx, severity_label in enumerate(severity_labels):
            ...
            sns.pointplot(
                ...
                dodge=.9,
                errorbar=("ci", 95),
                marker=[MARKER_MAP[strategy.split('_')[0]] for strategy in severity_df['strategy'].unique()],
            ...
            if row_idx == 0:
                ax.set_title(severity_label, fontsize=58)
            if row_idx == num_models - 1:
                ax.set_xlabel(x_label, fontsize=58)
            if col_idx == 0:
                ax.set_ylabel(model, fontsize=58)
    ...
    plt.tight_layout()
    plt.savefig(f'../results/{dataset}/{dataset}_miscoverage_results.pdf', bbox_inches='tight')
    plt.show()
