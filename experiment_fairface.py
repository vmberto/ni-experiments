from dataset.fairface import get_fairface_kfold_splits, get_fairface_dataset
...
def experiment():
    x_train, y_train, x_test, y_test, splits = get_fairface_kfold_splits(KFOLD_N_SPLITS)
    ...
    for index, config in enumerate(experiments_config):
        ...
        for fold, (train_index, val_index) in splits:
            ...
            train_ds = get_fairface_dataset(x_train_fold, y_train_fold, data_augmentation_layers)

            val_ds = get_fairface_dataset(x_val_fold, y_val_fold)
            test_ds = get_fairface_dataset(x_test, y_test)
